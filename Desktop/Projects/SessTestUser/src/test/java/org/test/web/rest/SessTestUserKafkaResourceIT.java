...
class SessTestUserKafkaResourceIT {
    ...
    void setup() {
        ...
        SessTestUserKafkaResource kafkaResource = new SessTestUserKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/sess-test-user-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/sess-test-user-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
