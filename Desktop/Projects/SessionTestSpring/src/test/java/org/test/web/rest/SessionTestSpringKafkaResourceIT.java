...
class SessionTestSpringKafkaResourceIT {
    ...
    void setup() {
        ...
        SessionTestSpringKafkaResource kafkaResource = new SessionTestSpringKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/session-test-spring-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/session-test-spring-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
